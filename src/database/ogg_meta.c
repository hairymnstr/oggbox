...
#define PLATprintf printf
#define PLATsscanf sscanf
...
int read_standard_tags(FILE *fr, struct meta *out) {
  ...
  fread(buf, 1, 1, fr);
  if(buf[0] != 3) {
    PLATprintf("not a comment packet :O\n");
    return -1;
  }
  fseek(fr, 6, SEEK_CUR);
  ...
  for(i=0;i<tagcount;i++) {
    ...
    if((strncmp(buf,"ARTIST", 6) == 0) && (mark == 6)) {
      ...
      if(PLATsscanf(&buf[mark+1], "%u", (unsigned int *)&utemp) == 1) {
        out->track = utemp;
      }      
    } else if((strncmp(buf, "DATE", 4) == 0) && (mark == 4)) {
      if(PLATsscanf(&buf[mark+1], "%d", &temp) == 1) {
        out->date = temp;
      }
