...
void usb_set_status(int);

volatile int _usb_status;
...
static void cdcacm_set_config(usbd_device *usbd_dev, uint16_t wValue)
{
	...
	usbd_register_control_callback(
        ...
        usb_set_status(USB_STATUS_ACTIVE);
        power_set_charge_full();
}

usbd_device *usbd_dev1;

int usb_get_status() {
  return _usb_status;
}

void usb_set_status(int status) {
  _usb_status = status;
}
...
//   static int usb_startup_counter = 0;
  if(power_get_usb_present()) {
    if(usb_get_status() == USB_STATUS_DOWN) {
      usb_set_status(USB_STATUS_STARTING);
//       usb_startup_counter = 1;
      vTaskDelay(10);
      gpio_clear(USB_DISC_PORT, USB_DISC_PIN);
      vTaskDelay(2000);
      if(usb_get_status() != USB_STATUS_ACTIVE) {
        // enumeration didn't happen within 2s, assume it's a charger only
        usb_set_status(USB_STATUS_CHARGER);
        power_set_charge_full();
      }
    }
  } else {
